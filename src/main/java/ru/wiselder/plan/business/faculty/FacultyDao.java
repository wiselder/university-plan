...
public class FacultyDao {
    ...
    private final String SELECT_GROUP_QUERY = """
            ...
            """;
    private final String SELECT_GROUP_BY_FACULTY = "SELECT * FROM GROUPS WHERE FACULTY_ID = :facultyId";
    private final String SELECT_GROUP_BY_ID = "SELECT * FROM GROUPS WHERE GROUP_ID = :id";

    private final RowMapper<Faculty> FACULTY_MAPPER = (rs, rowNum) -> new Faculty(
            rs.getInt("FACULTY_ID"),
            rs.getString("NAME")
    );
    private final RowMapper<Group> GROUP_MAPPER = (rs, rowNum) -> new Group(
            rs.getInt("GROUP_ID"),
            rs.getInt("FACULTY_ID"),
            rs.getInt("COURSE"),
            rs.getInt("NUMBER"),
    ...
    public List<Faculty> findAll() {
        return jdbcTemplate.query(SELECT_ALL, FACULTY_MAPPER);
    }
    ...
    public void createGroup(GroupRequest request) {
        jdbcTemplate.update(INSERT_GROUP_QUERY, mapOf(request));
    }

    public Optional<Group> findGroup(GroupRequest request) {
        return jdbcTemplate.query(SELECT_GROUP_QUERY, mapOf(request), SqlUtils.singleExtractor(GROUP_MAPPER));
    }
    ...
    private Map<String, Object> mapOf(GroupRequest request) {
        return Map.of(
